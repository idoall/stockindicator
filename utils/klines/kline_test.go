...
// RUN
// go test -v ./utils/klines -run TestToHeikinAshi
...
func TestItem_SortCandlesByTimestamp(t *testing.T) {
	...
	for x := 0; x < 100; x++ {
		...
		tempKline.Candles = append(tempKline.Candles,
			&Candle{
				TimeUnix: time.Now().AddDate(0, 0, -x).Unix(),
				Open:     y,
				High:     y + float64(x),
				Low:      y - float64(x),
				Close:    y,
	...
	if tempKline.Candles[0].TimeUnix > tempKline.Candles[1].TimeUnix {
...
func TestConvertToNewInterval(t *testing.T) {
	...
	old := &Item{
		...
		Candles: []*Candle{
			...
			{
				TimeUnix: time.Now().AddDate(0, 0, 2).Unix(),
				Open:     1696,
				High:     1998,
				Low:      1337,
				Close:    6969,
	...
	old.Candles = append(old.Candles, &Candle{
		TimeUnix: time.Now().AddDate(0, 0, 3).Unix(),
		Open:     6969,
		High:     1998,
		Low:      2342,
		Close:    7777,
	...
	old.Candles = []*Candle{
		...
		{
			TimeUnix: tn.AddDate(0, 0, 1).Unix(),
			Open:     1338,
			High:     2000,
			Low:      1332,
			Close:    1696,
		...
		{
			TimeUnix: tn.AddDate(0, 0, 2).Unix(),
			Open:     1696,
			High:     1998,
			Low:      1337,
			Close:    6969,
		...
		{
			TimeUnix: tn.AddDate(0, 0, 5).Unix(),
			Open:     6969,
			High:     8888,
			Low:      1111,
			Close:    5555,
		...
		{
			TimeUnix: tn.AddDate(0, 0, 6).Unix(),
			// Empty end padding
		},
		{
			TimeUnix: tn.AddDate(0, 0, 7).Unix(),
			// Empty end padding
		},
		{
			TimeUnix: tn.AddDate(0, 0, 8).Unix(),
			// Empty end padding
		},
	}
